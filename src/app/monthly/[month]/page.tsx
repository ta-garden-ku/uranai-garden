...
export default async function MonthlyDetailPage({ params }: Props) {
  ...
  return (
    <main className="page-shell space-y-8">
      <PageHero kicker={`MONTH ${item.month}`} title={item.name} description="今月のテーマ、恋愛運、仕事運、金運、ラッキー要素を演出つきで表示します。" />
      <FortuneRevealGate
        kicker="MONTHLY REVEAL"
        title={`${item.name}を開く`}
        description="月の流れを読み込んでから、今月のヒントを表示します。"
        buttonLabel="今月の結果を見る"
        readingLabel="月の流れを読み込んでいます..."
        variant="monthly"
      >

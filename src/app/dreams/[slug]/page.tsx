...
export default async function DreamDetailPage({ params }: Props) {
  ...
  return (
    <main className="page-shell space-y-8">
      ...
      />
      <PageHero kicker="DREAM DICTIONARY" title={`${dream.keyword}の夢占い`} description="夢の意味、恋愛・仕事・気持ちのヒントを、読み込み演出のあとに表示します。" />
      <AdSlot placement="article-top" />
      <FortuneRevealGate
        kicker="DREAM REVEAL"
        title={`${dream.keyword}の夢を読み解く`}
        description="夢の意味をすぐには出さず、少しだけ余韻を作ってから表示します。"
        buttonLabel="夢の意味を見る"
        readingLabel="夢の断片を読み込んでいます..."
        variant="dream"
      >

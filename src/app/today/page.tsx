...
import { todayKey } from "@/lib/fortune";
import { buildMetadata } from "@/lib/seo";

export const metadata = buildMetadata({
  title: "今日の運勢｜12星座の総合運・恋愛運・仕事運・金運",
  description: "12星座ごとの今日の運勢、ラッキーカラー、ラッキーアイテム、一言メッセージを毎日チェック。",
  path: "/today"
});
...
export default function TodayPage() {
  ...
  return (
    <main className="page-shell space-y-8">
      <PageHero kicker={date} title="今日の運勢" description="12星座の総合運、恋愛運、仕事運、金運、健康運をポジティブ寄りにお届けします。" />
            ...
            return (
              <article key={fortune.sign} className="soft-card">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="kicker">{sign?.period}</p>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <p><strong>恋愛運：</strong>{fortune.love}</p>
                  <p><strong>仕事運：</strong>{fortune.work}</p>
                  <p><strong>金運：</strong>{fortune.money}</p>
                  <p><strong>健康運：</strong>{fortune.wellness}</p>
                </div>
                <div className="mt-4 flex flex-wrap gap-2 text-sm">
                  <span className="rounded-full bg-honey/25 px-3 py-1 font-bold">色: {fortune.luckyColor}</span>
                  <span className="rounded-full bg-mintnight/10 px-3 py-1 font-bold text-mintnight">物: {fortune.luckyItem}</span>
                </div>
